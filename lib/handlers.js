									...
									userChecks.forEach( checkId => {
										...
										_data.delete('checks', checkId, err => {
											...
											if (checksDeleted === checksToDelete) {
												...
												cb(500, {'Error': 'Error encountered while' +
												' while attempting to delete all the user\'s' +
												' checks. All checks may not have been deleted' +
												' sucessfully'});
											}
										});
									});
			...
			if (!err && checkData) {
				...
				handlers._tokens.verifyToken(token, checkData.userPhone, 
					...
					if (tokenIsValid) {
						...
						_data.delete('checks', id, err => {
							if (!err) {
								_data.read('users', checkData.userPhone, 
									...
									if (!err && userData) {
										const userChecks = typeof(userData.checks) === 
											'object' && userData.checks instanceof Array ?
											userData.checks : [];
										...
										if (checkPosition > -1) {
											...
											//Пересохраняем данные пользователя
											_data.update('users', checkData.userPhone, userData, err => {
												if (!err) {
													cb(200);
												} else {
													cb(500, {'Error': 'Could not' +
														' update the user'});
												}
											});
										} else {
											cb(500, {'Error': 'Could not find the check on the user\'s object'});
										}
										
									} else {
										cb(500, {'Error': 'Could not find the user' +
						...
						cb(403, {'Error': 'Missing required token in headers' +
							' or token is invalid'});
					}
				...
				cb(400, {'Error': 'The specified id does not exists'});
			}
		});
